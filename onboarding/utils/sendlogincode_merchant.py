...
def SendMerchantVerificationCode(request, emailaddress):
    print('SendMerchantVerificationCode CALLED')
    try:
        if MerchantProfile.objects.filter(business_email = emailaddress).exists():
            userprofile = MerchantProfile.objects.get(business_email = emailaddress)
            ...
            if otp == 'CODE WAIT TIME':
                ...
                context = {'userName':business_name, 'otp':otp, 'emailAddress':emailaddress}
                try:
                    # SEND USING BREVO
